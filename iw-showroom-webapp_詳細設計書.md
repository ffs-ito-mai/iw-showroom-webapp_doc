...
* 現システムのページコンポーネントが使用している部品コンポーネントはtopPage 12個、detail 5個、search 3個あり、それぞれのコンポーネントが参照しているデータはJSファイルに直接記載されている。
* ID1-1では、それらのデータをCSVファイルに移行し、部品コンポーネントがCSVファイルからデータを参照するように変更を行う。
